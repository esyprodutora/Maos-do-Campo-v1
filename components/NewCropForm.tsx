  ...
  const SelectionCard = ({ selected, onClick, title, color, icon }: any) => (
    <div 
      ...
      className={`
        ...
        ${selected 
          ? `border-${color} bg-${color}/5 shadow-md scale-105` 
    ...
    >
      ...
      <div className={`p-3 rounded-full ${selected ? `bg-${color} text-white` : 'bg-gray-100 text-gray-500'}`}>
        {icon}
      </div>
      <span className={`font-bold ${selected ? 'text-gray-900' : 'text-gray-600'}`}>{title}</span>
    </div>
  );
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        ...
        <div className="bg-gray-50/50 p-6 border-b border-gray-100">
           <div className="flex items-center justify-between mb-6">
             <button onClick={onCancel} className="text-gray-400 hover:text-gray-600 flex items-center gap-1 font-medium text-sm">
             ...
             {steps.map((s) => (
               <div key={s.id} className="flex flex-col items-center gap-2 bg-gray-50 px-2">
                 <div className={`
                   ...
                   ${step >= s.id ? 'bg-agro-green text-white scale-110' : 'bg-white border-2 border-gray-200 text-gray-400'}
                 `}>
                   <s.icon size={18} />
                 </div>
        ...
        <div className="p-6 md:p-10 min-h-[400px]">
          {step === 1 && (
            <div className="space-y-8 animate-fade-in">
              <div className="text-center">
                 <h2 className="text-2xl font-bold text-gray-900">Vamos começar sua lavoura</h2>
                 <p className="text-gray-500">Escolha a cultura e defina o nome.</p>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Nome do Talhão / Lavoura</label>
                  <input 
                    type="text" 
                    value={formData.name}
                    onChange={e => setFormData({...formData, name: e.target.value})}
                    placeholder="Ex: Talhão da Baixada"
                    className="w-full p-4 bg-gray-50 border-2 border-transparent focus:bg-white focus:border-agro-green rounded-xl outline-none transition-all font-medium"
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Área Total (Hectares)</label>
                  <div className="relative">
                    <input 
                      ...
                      placeholder="0.0"
                      className="w-full p-4 pl-12 bg-gray-50 border-2 border-transparent focus:bg-white focus:border-agro-green rounded-xl outline-none transition-all font-medium"
                    />
          ...
          {step === 2 && (
            <div className="space-y-8 animate-fade-in">
              <div className="text-center">
                 ...
                 <p className="text-gray-500">Para um cálculo preciso de insumos.</p>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-3 text-center">Tipo de Solo</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Meta de Produtividade</label>
                  <div className="relative">
                    <input 
                      type="text" 
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Espaçamento</label>
                  <div className="relative">
                    <input 
                      type="text" 
          ...
          {step === 3 && (
            <div className="flex flex-col items-center justify-center text-center space-y-6 animate-fade-in">
               ...
               <div className="bg-gray-50 rounded-2xl p-6 w-full max-w-sm border border-gray-100 text-left relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-2 h-full bg-agro-green"></div>
                  <div className="space-y-3">
                     <div className="flex justify-between border-b border-gray-200 pb-2">
                       <span className="text-gray-500">Lavoura</span>
                     ...
                     <div className="flex justify-between">
                       ...
                       <span className="font-bold">{formData.areaHa} ha</span>
                     </div>
                  </div>
               </div>
            </div>
          ...
          <div className="ml-auto">
            {step < 3 ? (
              <button 
                ...
                className="px-8 py-3 bg-agro-dark text-white font-bold rounded-xl shadow-lg hover:bg-gray-800 transition-colors flex items-center"
              >
