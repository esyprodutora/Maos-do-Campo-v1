                                ...
                                <div className="space-y-4">
                                    {stage.resources?.filter(r => r.type === 'insumo').map(res => (
                                        <div key={res.id} className="text-sm group relative">
                                            ...
                                            {isEditingTimeline && (
                                              <button onClick={() => handleDeleteResource(stage.id, res.id)} className="absolute -right-2 -top-2 p-1 bg-red-100 text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={12}/></button>
                                            )}
                                        </div>
                                    ))}
