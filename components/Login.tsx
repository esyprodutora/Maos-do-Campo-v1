...
export const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex bg-white dark:bg-slate-900 overflow-hidden font-sans">
      ...
      {/* DESKTOP LEFT SIDE - VISUAL SLIDER (Hidden on mobile) */}
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12 relative overflow-y-auto">
        ...
        {/* LOGIN CARD - Glassmorphism on Mobile, Clean on Desktop */}
        <div className={`
            w-full max-w-md relative z-10 animate-fade-in
            lg:bg-transparent lg:p-0 
            bg-white/10 backdrop-blur-md border border-white/20 p-8 rounded-3xl shadow-2xl lg:shadow-none lg:border-none
        `}>
          ...
          <div className="mb-8 text-center lg:text-left">
            ...
            <div className="lg:hidden flex justify-center mb-6">
               <div className="bg-white/20 backdrop-blur-md p-3 rounded-2xl border border-white/30 shadow-lg">
                  <Leaf className="text-agro-green" size={32} fill="currentColor" />
               </div>
            </div>
            ...
            <p className="text-gray-200 lg:text-gray-500 lg:dark:text-gray-400 font-medium">
              {mode === 'signin' 
                ? 'Gerencie sua produção rural.' 
                : 'Junte-se a milhares de produtores.'}
            </p>
          </div>

          {!isSupabaseConnected && (
             <div className="mb-6 p-4 rounded-xl bg-red-500/20 border border-red-500/50 flex gap-3 items-start">
               <AlertCircle className="text-red-200" size={20} />
               <div className="text-sm text-white">
                 <p className="font-bold">Falha na Conexão</p>
                 <p className="mt-1 opacity-90">Verifique as variáveis <code>VITE_SUPABASE_URL</code>.</p>
               </div>
             </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <button 
              ...
              className="flex items-center justify-center gap-2 py-3 px-4 border border-white/20 lg:border-gray-200 lg:dark:border-slate-600 rounded-xl hover:bg-white/20 lg:hover:bg-gray-50 lg:dark:hover:bg-slate-700 transition-colors bg-white/10 lg:bg-white lg:dark:bg-slate-800 text-white lg:text-gray-700 lg:dark:text-gray-200 font-bold shadow-sm backdrop-blur-sm"
            >
            ...
            <button 
              ...
              className="flex items-center justify-center gap-2 py-3 px-4 bg-black/40 lg:bg-black text-white rounded-xl hover:bg-black/60 lg:hover:bg-gray-900 transition-colors font-bold shadow-sm backdrop-blur-sm border border-white/10 lg:border-transparent"
            >
          ...
          <div className="relative flex items-center mb-8">
             <div className="flex-grow border-t border-white/20 lg:border-gray-200 lg:dark:border-slate-700"></div>
             <span className="flex-shrink-0 mx-4 text-white/60 lg:text-gray-400 dark:text-gray-500 text-xs font-bold uppercase tracking-widest">Ou com email</span>
             <div className="flex-grow border-t border-white/20 lg:border-gray-200 lg:dark:border-slate-700"></div>
          </div>

          {msg && (
            <div className={`flex items-center gap-3 p-4 rounded-xl mb-6 text-sm font-medium border ${msg.type === 'error' ? 'bg-red-500/20 text-white border-red-500/50 lg:bg-red-50 lg:text-red-600 lg:border-red-100' : 'bg-green-500/20 text-white border-green-500/50 lg:bg-green-50 lg:text-green-600'}`}>
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            {mode === 'signup' && (
              <>
                <div className="space-y-1 animate-slide-up">
                  <label className="text-sm font-bold text-white/80 lg:text-gray-700 lg:dark:text-gray-300 ml-1">Nome Completo</label>
                  <div className="relative group">
                    <User className="absolute left-4 top-1/2 -translate-y-1/2 text-white/50 lg:text-gray-400 group-focus-within:text-agro-green transition-colors" size={20} />
                    <input 
                      type="text" 
                      required
                      value={fullName}
                      onChange={(e) => setFullName(e.target.value)}
                      className="w-full pl-12 pr-4 py-4 bg-white/10 lg:bg-gray-50 lg:dark:bg-slate-900 border border-white/10 lg:border-gray-200 lg:dark:border-slate-700 rounded-xl focus:bg-white/20 lg:focus:bg-white lg:dark:focus:bg-slate-800 focus:border-agro-green focus:ring-4 focus:ring-green-500/20 outline-none transition-all text-white lg:text-gray-900 lg:dark:text-white font-medium placeholder-white/30 lg:placeholder-gray-400"
                ...
                <div className="space-y-1 animate-slide-up" style={{animationDelay: '0.1s'}}>
                  <label className="text-sm font-bold text-white/80 lg:text-gray-700 lg:dark:text-gray-300 ml-1">WhatsApp</label>
                  <div className="relative group">
                    <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-white/50 lg:text-gray-400 group-focus-within:text-agro-green transition-colors" size={20} />
                    <input 
                      type="text" 
                      required
                      value={whatsapp}
            ...
            <div className="space-y-1">
              <label className="text-sm font-bold text-white/80 lg:text-gray-700 lg:dark:text-gray-300 ml-1">Email</label>
              <div className="relative group">
                <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-white/50 lg:text-gray-400 group-focus-within:text-agro-green transition-colors" size={20} />
                <input 
                  type="email" 
                  required
                  value={email}
            ...
            <div className="space-y-1">
               <div className="flex justify-between ml-1">
                 <label className="text-sm font-bold text-white/80 lg:text-gray-700 lg:dark:text-gray-300">Senha</label>
                 {mode === 'signin' && (
                   <a href="#" className="text-xs font-semibold text-agro-yellow lg:text-agro-green hover:underline">Esqueceu a senha?</a>
                 )}
               </div>
              <div className="relative group">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-white/50 lg:text-gray-400 group-focus-within:text-agro-green transition-colors" size={20} />
                <input 
                  type="password" 
                  required
                  value={password}
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full py-4 bg-agro-green hover:bg-green-600 lg:hover:bg-green-700 text-white font-bold rounded-xl shadow-lg shadow-green-900/30 lg:shadow-green-600/20 hover:shadow-green-900/50 transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 mt-6"
            >
              {loading ? (
          ...
          <div className="mt-8 text-center">
             <p className="text-white/70 lg:text-gray-500 lg:dark:text-gray-400 text-sm">
                ...
                <button 
                  ...
                  className="ml-2 font-bold text-agro-yellow lg:text-agro-green hover:underline focus:outline-none"
                >
